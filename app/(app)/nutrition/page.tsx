...
// Champs = nutriments clés pour le cheveu, à recopier depuis la capture MyFitnessPal.
const FIELDS = [
  { key: 'calories', label: 'Calories', unit: 'kcal' },
  { key: 'protein_g', label: 'Protein', unit: 'g' },
  { key: 'iron_mg', label: 'Iron', unit: 'mg' },
  { key: 'zinc_mg', label: 'Zinc', unit: 'mg' },
  { key: 'vit_d_iu', label: 'Vitamin D', unit: 'IU' },
...
export default function NutritionPage() {
  ...
  async function load() {
    if (!userId) return;
    setLoading(true);
    const { data } = await supabase.from('nutrition_logs').select('*').order('log_date', { ascending: false });
    setRows(data || []);
    setLoading(false);
  }
  ...
  async function add() {
    ...
    if (file) { screenshot_path = await uploadMedia(userId, 'nutrition', file); }
    const payload: any = { user_id: userId, log_date: date, screenshot_path, notes: notes.trim() || null };
    FIELDS.forEach((f) => { payload[f.key] = vals[f.key] !== undefined && vals[f.key] !== '' ? Number(vals[f.key]) : null; });
  ...
  return (
    <div>
      <div style={{ fontSize: 22, fontWeight: 700, color: '#1a1625' }}>Nutrition</div>
      <div style={{ fontSize: 14, color: '#8a8390', marginTop: 5, maxWidth: 640 }}>Attach your MyFitnessPal screenshot and copy the few nutrients that matter for hair. We track these — not every macro — so the analysis stays focused.</div>
      ...
      <div className="card" style={{ padding: 18, marginTop: 20 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: 10, marginTop: 14 }}>
          {FIELDS.map((f) => (
            <div key={f.key}>
              <div style={{ fontSize: 11.5, color: '#8a8390', marginBottom: 3 }}>{f.label} <span style={{ color: '#c3bdca' }}>({f.unit})</span></div>
            ...
            <motion.div key={r.id} className="card" initial={{ opacity: 0, x: -6 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.2, delay: i * 0.015 }} style={{ padding: '11px 14px' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginTop: 7 }}>
                {FIELDS.filter((f) => r[f.key] != null).map((f) => (
                  <span key={f.key} style={{ fontSize: 12.5, color: '#6b6573' }}><b style={{ color: '#1a1625' }}>{r[f.key]}</b> {f.unit} {f.label}</span>
                ))}
